...
if app_mode == "🔍 Item Search":
    ...
    if not item_options:
        ...
        # If the currently-selected item isn't among the search results
        # (e.g. the user just typed a new search term), jump to the first
        # match instead of leaving the detail panel on a stale item.
        valid_indices = set(item_options.values())
        if st.session_state.current_index not in valid_indices:
            st.session_state.current_index = next(iter(item_options.values()))
            save_persistent_state({"last_item_index": st.session_state.current_index})
